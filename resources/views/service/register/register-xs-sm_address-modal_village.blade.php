...
function {{$js_pullData}}(){
	...
	url = "{{ route("web.service.resource.region.villages",[false]) }}/" + $("#inp-district").val();
	$.get(url, function(data){
		...
		$.each(data, function(index, item){
			...
			.click(function(event){
				...
				$("#inp-village-label").val( $(this).html().trim() );
				$("#inp-village").val( $(this).attr("data-value").trim() ).trigger("change");
...
$(document).ready(function(){
	...
	$("#inp-district").on("change paste keyup", function(){
		$("#{{$js_filterId}}").val("");
...
<!-- Begin: st-regency -->
<input id="inp-village" 
	name="address_village" 
	type="hidden" 
	@if (isset($address_village))
		value="{{$address_village}}"
	@else
		value="{{old('address_village')}}"
	@endif
	/>
<input id="inp-village-label" 
	name="inp-village-label" 
	type="hidden" 
	value="{{old('inp-village-label')}}"
	/>
<div class="modal-body {{$stepClass}}">
	...
	<div class="input-group mt-3">
		...
		<input id="{{$js_filterId}}" type="text" class="form-control" autocomplete="off" placeholder="Kelurahan / Desa" />
	</div>
