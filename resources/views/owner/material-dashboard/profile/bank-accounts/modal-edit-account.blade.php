	...
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			...
			<div class="modal-body border-0 p-4">
				<form id="{{$formID}}" 
					class="mb-0"
					action="{{route('web.owner.profile.bank.edit')}}" 
					method="post">
					@csrf
					<input name="acc[id]" type="hidden" value="{{md5($acc->id)}}" />
					...
					<div class="{{$formID}}-div-details">
						...
						<div class="mt-2">
							@php 
								$errorID = "_".\Str::random();
							@endphp
							<input id="{{$formID}}-inp-acc" 
								name="acc[bank_account]" 
								old-value="{{$acc->bank_account}}"
								class="form-control" 
								type="text" 
...
function {{$formID}}_defaultValue(){
	console.log('_defaultValue');
	$('#{{$formID}}').find('input').each(function(ind, item){
		let oldVal = $(item).attr('old-value');
		if (oldVal){
			$(item).val(oldVal);
		}
	});
